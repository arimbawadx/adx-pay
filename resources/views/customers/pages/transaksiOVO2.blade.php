...
@section('title','adx-pay | Transaksi OVO')

@section('content-header', 'Transaksi OVO')
  ...
  <div class="container">
    <div class="row">
      <div class="col-md-6 col-12">
        <form class="card card-body" id="form-confirm" method="post" action="/customers/transaksi/ovo/3">
          {{csrf_field()}}
          <input name="inquiry" value="{{$inquiry}}" type="hidden">
          <input name="no_hp" value="{{$phone}}" type="hidden">
          <div class="form-group">
            <label for="code">Jumlah OVO</label>
            <select id="code" name="code" required class="form-control">
              ...
              <option value="{{$hargapulsa['code']}}">{{$hargapulsa['description']}}  |  
                ...
                Harga Rp. 502.000
                @elseif($hargapulsa['code']==$inquiry.'600')
                Harga Rp. 602.000
                @elseif($hargapulsa['code']==$inquiry.'700')
                Harga Rp. 702.000
                @elseif($hargapulsa['code']==$inquiry.'800')
                Harga Rp. 802.000
                @elseif($hargapulsa['code']==$inquiry.'900')
                Harga Rp. 902.000
                @elseif($hargapulsa['code']==$inquiry.'1000')
                Harga Rp. 1.002.000
                @endif
              </option>
              @endif
